...
import { AuthMiddleware, BeforeReqMiddleware, CacheMiddleware } from 'init/Middleware'
...
setInterval(() => {
  ...
  console.clear()
  const state = test.State === CBState.Closed ? 'Закрыт' : (test.Timeout || 0) < Date.now() ? 'Полу' : 'Открыт'
